...
export function renderToString(element, $) {
  if (!element) return ""
    ...
    // @ts-ignore
    if (element.$$typeof === 'html') return element.data
    else {
      if (typeof element.type == "symbol") return renderToString(element.props?.children, $)
    ...
    return sanitized[element + ''] || (sanitized[element + ''] = esc(element + ''))
